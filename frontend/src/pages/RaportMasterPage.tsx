...
interface PengajianInstrument {
  instrumen_id: number;
  jenis_pengajian: 'AL_QURAN' | 'TAKHASSUS';
...
export function RaportMasterPage() {
  usePageMeta({
    title: 'Master Kriteria Pengajian',
    description: 'Kelola kriteria metrik penilaian pengajian santri (PBS Al-Qur\'an & PBM Kitab) serta rentang nilai A–E.',
  });

  const queryClient = useQueryClient();

  // Active Main Tab: 'AL_QURAN' | 'TAKHASSUS' | 'RENTANG_NILAI'
  const [activeTab, setActiveTab] = useState<'AL_QURAN' | 'TAKHASSUS' | 'RENTANG_NILAI'>('AL_QURAN');

  // Search and filter for criteria tabs
  const [searchQuery, setSearchQuery] = useState('');
  const [statusFilter, setStatusFilter] = useState<'all' | 'active' | 'inactive'>('all');

  // Modals state for criteria
  const [showAddModal, setShowAddModal] = useState(false);
  const [newCriteriaName, setNewCriteriaName] = useState('');

  const [editingInstrument, setEditingInstrument] = useState<PengajianInstrument | null>(null);
  ...
  // Fetch criteria list
  const { data: instruments = [], isLoading: isLoadingInstruments, isError: isErrorInstruments, refetch: refetchInstruments } = useQuery<PengajianInstrument[]>({
    queryKey: ['raport-pengajian-master'],
    queryFn: async () => (await api.get('/api/raport-pengajian/master')).data,
  });

  // Fetch rentang nilai
  const { data: initialRanges = [], isLoading: isLoadingRanges, refetch: refetchRanges } = useQuery<RentangNilaiItem[]>({
    queryKey: ['rentang-nilai-pengajian'],
    queryFn: async () => (await api.get('/api/rentang-nilai/pengajian')).data,
  ...
  const addMutation = useMutation({
    mutationFn: async ({ jenis, nama }: { jenis: 'AL_QURAN' | 'TAKHASSUS'; nama: string }) => {
      return (await api.post('/api/raport-pengajian/master', {
    ...
    onSuccess: () => {
      ...
      void queryClient.invalidateQueries({ queryKey: ['raport-pengajian-master'] });
      void queryClient.invalidateQueries({ queryKey: ['raport-pengajian-options'] });
    },
    onError: (err: any) => {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, nama }: { id: number; nama: string }) => {
      return (await api.put(`/api/raport-pengajian/master/${id}`, { nama_instrumen: nama })).data;
    },
    onSuccess: () => {
  ...
  const toggleMutation = useMutation({
    mutationFn: async (id: number) => {
      return (await api.patch(`/api/raport-pengajian/master/${id}/toggle`)).data;
    },
    onSuccess: () => {
  ...
  const saveRangesMutation = useMutation({
    mutationFn: async (items: RentangNilaiItem[]) => {
      return (await api.put('/api/rentang-nilai/pengajian', { ranges: items })).data;
    },
    onSuccess: () => {
      setRangeError('');
      showToast('Rentang nilai A–E pengajian berhasil disimpan');
      void queryClient.invalidateQueries({ queryKey: ['rentang-nilai-pengajian'] });
    },
    onError: (err: any) => {
  ...
  // Filtered criteria based on activeTab (AL_QURAN or TAKHASSUS)
  const filteredInstruments = useMemo(() => {
    if (activeTab === 'RENTANG_NILAI') return [];
    return instruments.filter(item => {
      if (item.jenis_pengajian !== activeTab) return false;
  ...
  }, [instruments, activeTab, searchQuery, statusFilter]);

  // Current tab instruments count
  const currentTabInstruments = useMemo(() => {
    if (activeTab === 'RENTANG_NILAI') return [];
    return instruments.filter(i => i.jenis_pengajian === activeTab);
  ...
  const openEditModal = (inst: PengajianInstrument) => {
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    ...
    if (activeTab === 'RENTANG_NILAI') return;
    addMutation.mutate({ jenis: activeTab, nama: newCriteriaName.trim() });
  };
  ...
  return (
    <section style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 8px 48px 8px', background: 'transparent' }}>
      ...
      <div className="cms-master-header">
        <div>
          ...
          <p style={{ color: '#64748b', fontSize: '14px', margin: 0 }}>
            Kelola metrik penilaian pengajian santri (PBS Al-Qur'an &amp; PBM Kitab) serta rentang nilai A–E.
          </p>
        </div>
        ...
        {activeTab !== 'RENTANG_NILAI' && (
          ...
          >
            ...
            Tambah Kriteria {activeTab === 'AL_QURAN' ? 'Al-Qur\'an' : 'Kitab'}
          </button>
        )}
      </div>

      {/* Main Tab Switcher */}
      <div style={{ display: 'flex', gap: '8px', margin: '16px 0 20px 0', borderBottom: '1px solid #cbd5e1', paddingBottom: '12px' }}>
        <button
          type="button"
          onClick={() => { setActiveTab('AL_QURAN'); setSearchQuery(''); setStatusFilter('all'); }}
          style={{
            padding: '8px 18px',
            fontSize: '13.5px',
            fontWeight: activeTab === 'AL_QURAN' ? 700 : 500,
            borderRadius: '6px',
            border: '1px solid',
            borderColor: activeTab === 'AL_QURAN' ? '#0f6e56' : '#cbd5e1',
            background: activeTab === 'AL_QURAN' ? '#0f6e56' : '#ffffff',
            color: activeTab === 'AL_QURAN' ? '#ffffff' : '#334155',
            cursor: 'pointer',
            transition: 'all 0.15s ease',
          }}
        >
          Pengajian Al-Qur'an (PBS)
        </button>
        <button
          type="button"
          onClick={() => { setActiveTab('TAKHASSUS'); setSearchQuery(''); setStatusFilter('all'); }}
          style={{
            padding: '8px 18px',
            fontSize: '13.5px',
            fontWeight: activeTab === 'TAKHASSUS' ? 700 : 500,
            borderRadius: '6px',
            border: '1px solid',
            borderColor: activeTab === 'TAKHASSUS' ? '#0f6e56' : '#cbd5e1',
            background: activeTab === 'TAKHASSUS' ? '#0f6e56' : '#ffffff',
            color: activeTab === 'TAKHASSUS' ? '#ffffff' : '#334155',
            cursor: 'pointer',
            transition: 'all 0.15s ease',
          }}
        >
          Pengajian Kitab (PBM / Takhassus)
        </button>
        <button
          type="button"
          ...
          style={{
            ...
            borderRadius: '6px',
            border: '1px solid',
            borderColor: activeTab === 'RENTANG_NILAI' ? '#0f6e56' : '#cbd5e1',
            background: activeTab === 'RENTANG_NILAI' ? '#0f6e56' : '#ffffff',
            color: activeTab === 'RENTANG_NILAI' ? '#ffffff' : '#334155',
            cursor: 'pointer',
            transition: 'all 0.15s ease',
          }}
        >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
              <div className="cms-search-wrapper">
                ...
                <input
                  ...
                  placeholder={`Cari kriteria ${activeTab === 'AL_QURAN' ? 'Al-Qur\'an' : 'Kitab'}...`}
                  value={searchQuery}
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                ...
                >
                  Semua ({currentTabInstruments.length})
                </button>
                <button
                  type="button"
                ...
                >
                  Aktif ({currentTabInstruments.filter(i => i.status_aktif).length})
                </button>
                <button
                  type="button"
                ...
                >
                  Nonaktif ({currentTabInstruments.filter(i => !i.status_aktif).length})
                </button>
              </div>
            </div>

            <div style={{ fontSize: '13px', color: '#64748b' }}>
              Menampilkan <strong>{filteredInstruments.length}</strong> dari {currentTabInstruments.length} kriteria
            </div>
          </div>
          ...
          {isLoadingInstruments ? (
            ...
            <div style={{ padding: '32px', textAlign: 'center' }}>
              <p style={{ color: '#b91c1c', marginBottom: '12px' }}>
                Gagal memuat kriteria pengajian. Silakan periksa koneksi lalu coba lagi.
              </p>
              <button type="button" className="secondary-button" onClick={() => void refetchInstruments()}>
                Coba Lagi
              </button>
            </div>
          ) : (
        ...
        <div className="cms-master-card" style={{ border: '1px solid #cbd5e1', borderRadius: '12px', background: '#ffffff', overflow: 'hidden', padding: '24px' }}>
          <div style={{ marginBottom: '20px' }}>
            <h2 style={{ fontSize: '18px', fontWeight: 700, color: '#0f172a', margin: '0 0 6px 0' }}>
              Konfigurasi Rentang Nilai A–E (Pengajian)
            </h2>
            <p style={{ fontSize: '13.5px', color: '#64748b', margin: 0 }}>
              Atur batas nilai minimum, maksimum, dan predikat resmi untuk setiap huruf kategori penilaian pengajian.
            </p>
          </div>
          ...
          <div className="cms-modal-box" onClick={e => e.stopPropagation()}>
            <div className="cms-modal-header">
              <h2 className="cms-modal-title">
                Tambah Kriteria {activeTab === 'AL_QURAN' ? 'Al-Qur\'an (PBS)' : 'Kitab (PBM)'}
              </h2>
              <button
                type="button"
            ...
            <form onSubmit={handleAddSubmit}>
              <div className="cms-modal-body">
                ...
                <input
                  type="text"
                  autoFocus
                  placeholder="Contoh: Makharijul Huruf / Murod"
      ...
      {/* MODAL 2: UBAH / GANTI NAMA METRIK */}
      {editingInstrument && (
